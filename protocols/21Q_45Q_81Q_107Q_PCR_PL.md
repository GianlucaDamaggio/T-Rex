...
### *straglr*, for CAG count using the Whole-genome as reference
...
### Merge multiple *straglr* output - version CAG_v4.bed - WG

```bash
cd /lustrehome/gianluca/straglr/output/AC2021_NANOPORE_PCR10/21Q_45Q_81Q_107Q_PCR_PL

for n in 01 02 03 04 05 06 07 08 ; do ls barcode${n}_WG_v4_split*.tsv | grep aa | while read file ; do cat ${file} > barcode${n}_WG_v4.tsv; done ; done

for n in 01 02 03 04 05 06 07 08 ; do ls barcode${n}_WG_v4_split*.tsv | grep -v aa | while read file ; do cat ${file} | grep -v "#" >> barcode${n}_WG_v4.tsv; done ; done

#cancel all .ok files created for job-control
rm *.ok
```

### Create folder to save on local computer with blastn+straglr+alignment_summary - version CAG_v4.bed - WG

```bash
cd /lustrehome/gianluca/straglr/output/AC2021_NANOPORE_PCR10/21Q_45Q_81Q_107Q_PCR_PL

for n in 01 02 03 04 05 06 07 08 ; do mkdir barcode${n}_WG_v4 ; done
...
for n in 01 02 03 04 05 06 07 08 ; do cp ${ALIGNMENT}/barcode${n}_WG/alignment_summary.txt ${STRAGLR}/barcode${n}_WG_v4/ &&  cp ${BLASTN}/barcode${n}* ${STRAGLR}/barcode${n}_WG_v4/ &&  cp ${STRAGLR}/barcode${n}_WG_v4.tsv ${STRAGLR}/barcode${n}_WG_v4/ ; done 
```

### Rsync to local computer - version CAG_v4.bed - WG

```bash
mkdir /Users/gianlucadamaggio/projects/cattaneo/straglr/AC2021_NANOPORE_PCR10/21Q_45Q_81Q_107Q_PCR_PL/

for n in 01 02 03 04 05 06 07 08 ; do rsync -avh --progress gianluca@ui02.recas.ba.infn.it:/lustrehome/gianluca/straglr/output/AC2021_NANOPORE_PCR10/21Q_45Q_81Q_107Q_PCR_PL/barcode${n}_WG_v4 /Users/gianlucadamaggio/projects/cattaneo/straglr/AC2021_NANOPORE_PCR10/21Q_45Q_81Q_107Q_PCR_PL/ ; done
```
